...
class ShelterService {
  ...
  /// Optional admin helper: approve a pending shelter.
  Future<void> approveShelter(String id) async {
    try {
      await _col.doc(id).set(
        {
          'status': 'active',
      ...
      debugPrint('Failed to approve shelter: $e');
    }
  }

  /// Optional admin helper: reject a pending shelter.
  Future<void> rejectShelter(String id) async {
    try {
      await _col.doc(id).set(
        {
          'status': 'removed',
      ...
      debugPrint('Failed to reject shelter: $e');
    }
  }
  ...
  Future<String> uploadShelterImage({
  ...
  Shelter _fromDoc(DocumentSnapshot<Map<String, dynamic>> doc) {
    ...
    return Shelter(
      ...
      galleryImages: List<String>.from(
        (data['gallery_images'] ?? const <dynamic>[]) as List,
      ),
      rating: (data['rating'] ?? 0) is num
